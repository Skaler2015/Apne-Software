...
<div class="modal-overlay" id="toolModal">
  <div class="modal-box">
    ...
    <div class="mt-row2">
      ...
      <div class="field"><label for="toolCategory">Category *</label><select id="toolCategory"></select></div>
    </div>
...
function renderCategoryOptions(){
  const toolCategorySel = document.getElementById('toolCategory');
  const catFilterSel = document.getElementById('catFilter');
  toolCategorySel.innerHTML = toolsData.categories.map(c => `<option value="${c.id}">${c.icon} ${c.name}</option>`).join('');
...
function renderToolsTable(){
  ...
  tbody.innerHTML = rows.map(tool => {
    ...
    const catLabel = cat ? `${cat.icon} ${cat.name}` : tool.category;
...
document.getElementById('addToolBtn').addEventListener('click', ()=>{
  ...
  if(toolsData.categories.length) document.getElementById('toolCategory').value = toolsData.categories[0].id;
  openModal('toolModal');
});
...
document.getElementById('toolSaveBtn').addEventListener('click', ()=>{
  ...
  if(!category){ showToast('⚠️ Please create a category first'); return; }

  if(editId){
    const tool = toolsData.tools.find(t => t.id === editId);
    Object.assign(tool, { name, description: desc, icon, category, url, published });
  } else {
    const existingIds = toolsData.tools.map(t => t.id);
    const id = uniqueId(name, existingIds);
    toolsData.tools.push({ id, name, description: desc, icon, category, url, published });
  }
  renderToolsTable();
...
function deleteCat(id){
  const cat = toolsData.categories.find(c => c.id === id);
  if(!cat) return;
  const count = toolsData.tools.filter(t => t.category === id).length;
  const warning = count > 0
    ? `This category has ${count} tool(s) in it. Deleting it will leave those tools without a category. Delete anyway?`
